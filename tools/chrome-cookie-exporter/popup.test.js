...
test("popup markup documents the runtime bundle as the default recovery path", () => {
  assert.match(popupHtml, /默认推荐/u);
  assert.match(popupHtml, /运行时恢复/u);
  assert.match(popupHtml, /诊断摘要/u);
  assert.match(popupHtml, /刷新|滑块/u);
  assert.match(popupHtml, /再次导出/u);
...
test("popup styles keep warnings visually distinct and export text selectable", () => {
  assert.match(popupCss, /\.warnings-panel/u);
  assert.match(popupCss, /border:\s*1px solid[^;]+#/u);
  assert.match(popupCss, /background:[^;]+#/u);
...
test("popup initialize auto-copies the default runtime bundle on a supported page", async () => {
  ...
  assert.equal(document.elements.status.textContent, "已复制运行时恢复包到剪贴板。");
  assert.equal(
  ...
  assert.equal(document.elements.toggleExportModeButton.textContent, "改为导出诊断摘要");
  assert.equal(document.elements.retryWithoutValidationButton.textContent, "当前页不符时，忽略页面校验后重试");
  assert.equal(
  ...
  assert.deepEqual(getWarningTexts(document), [
    "当前结果缺少推荐补充 Cookie: tfstk, _m_h5_tk_enc",
  ]);
});
...
test("popup copy button re-copies the current export text", async () => {
  ...
  assert.equal(
    document.elements.readinessSummary.textContent,
    "飞书入口识别：已满足；运行时关键项：缺少 XSRF-TOKEN；推荐补充项：缺少 x5sec, tfstk, _m_h5_tk_enc",
  );
  assert.deepEqual(getWarningTexts(document), [
    "当前结果缺少运行时关键 Cookie: XSRF-TOKEN",
    "当前结果缺少推荐补充 Cookie: x5sec, tfstk, _m_h5_tk_enc",
  ]);
});
...
test("popup shows copy failure warning while keeping the exported text", async () => {
  ...
  assert.equal(document.elements.status.textContent, "已生成运行时恢复包，但自动复制失败。");
...
test("popup blocks unsupported pages until retry without validation is clicked", async () => {
  ...
  assert.match(document.elements.status.textContent, /请先打开闲鱼相关页面/u);
  ...
  assert.equal(document.elements.status.textContent, "已复制运行时恢复包到剪贴板。");
  assert.equal(calls.cookiesGetAll.length, 3);
});
...
test("popup profile toggle switches to the diagnostic summary mode", async () => {
  ...
  assert.equal(document.elements.toggleExportModeButton.textContent, "改为导出运行时恢复包");
